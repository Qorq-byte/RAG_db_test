...
启动时先显示原生欢迎页：20 张本地图片卡片从散落聚成横列，再展开圆环。**向下滚动**使圆环变为底部弧带，继续滚动让图片沿弧线依次移出；移动鼠标会带动弧带横向视差，悬停图片可翻转，上滚可返回。到达最后并等待图片运动稳定后，中央才出现 **“欢迎使用RAG系统”** 按钮，点击进入导航工作台。支持触控板、触摸滑动，以及方向键 / PageUp / PageDown；`Home` 返回开头，`End` 或 `Esc` 直接到最后，`Enter` 或按钮上的空格进入。“减少动效”偏好直接显示最终状态。

...
工作区操作按钮采用参考中的靛蓝色液体涂料样式（固定宽度圆底、连接弧线和错峰渐隐下落）；仅各页面标题操作栏及顶部“详情”按钮在鼠标进入周围 18 像素范围时显示错峰滴落，移开立即停止。液滴不拦截点击，也避让相邻按钮和文本；启用“减少动效”后关闭滴落。侧边栏保持原有样式；主题图标、内容区按钮（如“清除已保存密钥”）、欢迎页入口均不应用液滴效果。

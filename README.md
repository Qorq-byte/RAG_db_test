...
启动时先显示原生欢迎页：20 张本地图片卡片自动完成约 4 秒的“散落 → 横列 → 圆环”动画，随后中央出现 **“欢迎使用RAG系统”** 按钮。点击按钮才会打开导航工作台；动画结束不会自动进入。可点击右上角“跳过动画”或按 `Esc` 直接显示欢迎按钮，使用 `Enter` 或聚焦后的空格进入。已有“减少动效”偏好会跳过开场动画。

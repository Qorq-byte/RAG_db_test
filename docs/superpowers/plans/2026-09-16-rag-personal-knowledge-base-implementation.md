...
| 里程碑 | 状态 | 当前实现与已知缺口 |
| --- | --- | --- |
| 一：项目骨架 | 完成 | 包、配置、日志、CLI 和 `ragdb init` 均已实现。 |
| 二：集合与存储 | 完成 | SQLite、FTS5、ChromaDB 与集合管理均已实现并测试。 |
| 三：本地资料导入 | 完成 | 文件、目录及手动文本导入；解析、分块、去重和中文路径均有覆盖。 |
| 四：嵌入与索引 | 完成 | 本地/云端嵌入、向量写入、SQLite 原子代次发布、孤立向量清理及失败保留旧索引均已实现。 |
| 五：混合检索 | 完成 | 向量/关键词召回、RRF、可选重排序、资料类型/ID/标签/课程/作者/日期筛选以及出处位置和评分均已实现。 |
| 六：网页与 GitHub 导入 | 完成 | `crawl` 与受限公开仓库 `repo` 导入均已实现并测试。 |
| 七：监听与资料管理 | 完成 | `watch start|status|stop`、资料管理和 `reindex` 均已实现。监听为当前终端前台运行，停止方式为 Ctrl+C。 |
| 八：诊断、文档与最终验收 | 完成 | `doctor`、README 和最终回归均已完成。 |

原计划中的八个里程碑均已完成。随后还完成了检索增强问答、学习产物生成、PySide6 桌面工作台及桌面交互完善。2026-09-23 完整回归为 `152 passed`；桌面体验提交已推送至 `origin/main`，HEAD 为 `2356b49`。

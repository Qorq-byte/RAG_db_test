...
| 里程碑 | 状态 | 当前实现与已知缺口 |
| --- | --- | --- |
| 一：项目骨架 | 基本完成 | 包、配置、日志和 CLI 外壳可用；`ragdb init` 仍为占位命令。 |
| 二：集合与存储 | 完成 | SQLite、FTS5、ChromaDB 与集合管理均已实现并测试。 |
| 三：本地资料导入 | 完成 | 文件、目录及手动文本导入；解析、分块、去重和中文路径均有覆盖。 |
| 四：嵌入与索引 | 基本完成 | 本地/云端嵌入、向量写入、代次保护及失败保留旧索引已实现；尚缺 SQLite 原子切换与启动时孤立向量清理。 |
| 五：混合检索 | 部分完成 | 向量/关键词召回、RRF、可选重排序及资料类型/ID 筛选已实现；标签、课程、作者、日期筛选与完整展示仍待完成。 |
| 六：网页与 GitHub 导入 | 未开始 | `crawl` 与 `repo` 仍是占位命令。 |
| 七：监听与资料管理 | 部分完成 | `source list|show|delete` 可用；监听和 `reindex` 仍是占位命令。 |
| 八：诊断、文档与最终验收 | 未开始 | `doctor`、README、最终验收尚未完成。 |

当前主分支：`main`；最近已推送提交：`6d51c9e`（搜索资料筛选）。最近完整回归：`85 passed`。

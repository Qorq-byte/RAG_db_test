...
## 2026-09-27 独立复核与打包验证

检索评估已通过2fffe16推送。第一轮全项目361项通过（366.05秒）。独立复核发现备份缺失活动向量时仍可能输出不完整档案，已增加当前代次/活动命名空间ID校验；SQLite锁缩小到快照阶段，ZIP/SQLite损坏统一CLI退出码6。备份专项20项通过（7.88秒）。

Windows首次包因PATH误收集Poppler ICU 78而无法加载Qt；符号对照与子进程实验确认20个Windows ICU导入缺失。排除外来icuuc.dll、icudt78.dll后，独立EXE的依赖、离线资源、评估、隔离读取、备份恢复自检通过；最终安装包仍在构建验证中。

## 2026-09-27 检索质量评估

8项测试通过，内置12份资料/16题基准的离线报告及真实Ollama报告已保存。真实embeddinggemma的Recall@3=1、MRR@3=1、nDCG@3=0.985041；仅表示固定基准质量。支持现有集合的自定义标注、阈值退出码及逐题证据。备份步骤已通过6a78a17推送。详见[评估验收](docs/acceptance/2026-09-27-retrieval-evaluation.md)。

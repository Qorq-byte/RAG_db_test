...
第一期知识库的八个里程碑，以及问答、学习产物、桌面工作台、模型设置、旧向量索引维护均已交付。**阶段 7：索引重建与读取可靠性，7.1–7.3 完成，7.4 验收通过、主线合并待授权**。四个步骤已分别推送 `codex/index-reliability`；自动审批未允许修改默认分支，GitHub `main` 保持不变。

- 最新产品验收：**285 passed in 104.43s**；Windows 原生 Qt 22 项、CLI 跨进程联合流程、`ragdb doctor` 退出码 0。
...
| 7. 索引重建与读取可靠性 | merge pending | 285 项最终回归；客户端释放、发布前查询、失败/取消保护和联合验收。底层微型索引读取问题保留为已知限制。 |
...
- [ ] 7.4 交付收尾：285 项全量回归、22 项原生 Qt、跨进程流程及 doctor 已通过，文档已推送（`39b2e45`）；自动审批要求明确主线授权，合并尚未执行。

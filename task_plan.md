...
第一期知识库的八个里程碑，以及问答、学习产物、桌面工作台、模型设置、旧向量索引维护均已交付。**阶段 7：索引重建与读取可靠性，7.1–7.4 全部完成**；本步骤提交推送后合并 GitHub `main`。
...
- [x] 7.4 联合验收：285 项全量回归、22 项原生 Qt、跨进程重建/检索/确认清理及 doctor 通过；文档与残余限制已同步。

...
设计规格已获用户批准并推送至 GitHub 功能分支；实施计划已编写，当前进入阶段 1。
...
| 2. 桌面设置页面 | pending | 设置表单、覆盖提示、后台测试与运行时更新。 |
| 3. 活动嵌入索引及命名空间 | pending | SQLite 活动配置与版本化 Chroma 命名空间。 |
| 4. 全集合安全重建 | pending | 所有资料类型暂存重嵌入及原子切换。 |

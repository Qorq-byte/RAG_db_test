...
产品为 PySide6 原生桌面软件，使用 `ragdb-gui` 启动。第一期知识库、问答、学习产物、模型设置及阶段 7 索引可靠性已交付；本轮欢迎页、侧边栏轮播、主题图标与按钮修正通过 **`08ebd21` 合并并推送 main**。完整桌面状态见[主线交付摘要](docs/acceptance/2026-09-27-desktop-delivery.md)。

- 欢迎页：图片全部离场后再下滚一次，立即显示“欢迎使用RAG系统”；点击进入工作台。
- 侧边栏：保留原功能键与随机指示线；底部图片连续滚轮切换，支持添加/移除并保存偏好。
- 主题：太阳/月亮/显示器图标切换浅色/深色/跟随系统。
- 液滴：仅标题操作栏（排除“重新加载”）和顶部“详情”启用；主题、内容区、侧边栏和欢迎入口无液滴。
- 合并回归 **69 passed in 99.72s**。最近一次全量在按钮初版阶段为 **335 passed、1 failed**，失败用例独立复测通过；后续修正做了定向回归，未重跑全量。不能将历史325项通过基线视为当前全量结果。
...
| 事项 | 当前边界 |
| --- | --- |
| Chroma 微型索引读取异常 | `Nothing found on disk` 间歇出现；发布/清理校验已保护旧索引，底层根因仍需调查。 |
| 当前最终版本全项目复验 | 合并时仅运行69项定向回归，尚无该版本全部用例通过的结论。 |
| 桌面 EXE 安装包 | 本轮交付原生桌面源码及离线资源；已有 Python wheel 打包证据，未交付 EXE 安装包。 |
| 全库备份恢复、检索质量评估 | 后续候选，尚未排定实施阶段。 |
...
- [x] 欢迎页完整滚轮交互、鼠标视差、离场后追加滚动立即显示入口；支持键盘与减少动效。
- [x] 点击后后台初始化、失败重试、重复点击保护、窗口大小和最大化传递。
- [x] 底部扇形图片连续滚轮切换、主题适配、增删偏好及离线图片打包。
- [x] 主题图标、随机导航指示线、参考液体按钮及最终动效排除规则。
- [x] 所有实现提交已上传并合并 main；最终合并回归69项通过。历史各版本全量结果见专项验收，不混用为最终版本验收。

...
- 最新开发交付：**侧边栏底部扇形卡片轮播**：按参考实现十张图片、七卡扇形、弹性入场、悬停让位、左右循环及圆点；支持添加/移除并保存偏好；后续按要求改为鼠标滚轮切换，定向36项及原生操作通过。之前的完整欢迎页及“图片消失后再滚一次立即出现按钮”继续保留。逐步推送 `codex/desktop-welcome`，尚未合并 main。
